...
<pre>
	1. session : 상태 유지를 위한 정보가 서버측에 저장되는 개념.
		세션(HttpSession)
			...
			http -> connectionLess --> 시간(웹앱을 사용하기 시작하는 순간부터~종료 이벤트가 발생할때까지의 기간)의 의미로 세션을 표현함.
		세션의 생명주기
			...
			유지 방법 : 서버와 클라이언트가 동일한 세션 아이디를 공유하는 상황. (tracking mode) 세션이미지 
				...
				2) URL : request line을 통한 세션 아이디 공유(보안에 취약함)
					<a href="stateFull.jsp;jsessionid=<%=session.getId()%>">세션 파라미터를 통한 세션 유지</a>
				3) SSL : Seqcure Socket Layer(https) 를 통해 암호화 전송 구조를 이용한 공유.
			소멸 : 만료 시간(timeout)이내에 새로운 요청이 발생하지 않는 경우.
				1) 브라우저 종료
				2) 쿠키 삭제
				3) 직접 로그아웃(session invalidate) 했을 경우
				<%
// 					session.invalidate();
				%>
	2. cookie : 상태 유지를 위한 정보가 클라이언트측에 저장되는 개념.
		1) 쿠키 객체 생성 : name/value 속성, value에 한글을 비롯한 특수문자가 포함된 경우 -> URLencoding 과정이 필요함.
		2) 쿠키의 부가 속성 설정.
			- name/value(required) 
			...
			- path : 생략한 경우, 쿠키를 생성할때 경로가 반영됨
					 쿠키의 재전송 여부를 결정하는 조건.
					 ex) / : 특정 도메인 이후 모든 경로로 발생한 요청에 재전송됨.
					 	 request.getContext() : 특정 도메인의 해당 컨텍스트로 발생한 요청에 재전송됨
					 	 request.getContext()/10 : 특정 도메인의 해당 컨텍스트 "/10" 경로로 발생한 요청에 재전송됨
			- maxAge : 쿠키의 저장 시한 결정(초단위).
						ex) 60 * 60 * 24 * 7
							0 : 쿠키 삭제
							-1 : 브라우저 종료시 쿠키 삭제
			- httpOnly : 클라이언트 사이드 모듈에서의 접근을 허용하지 않을때.
			- secure : SSL(https) 프로토코로에서만 쿠키를 재전송하는 구조.
		3) response.addCookie : 응답 헤더(set-cookie)를 통해 클라이언로 전송.
