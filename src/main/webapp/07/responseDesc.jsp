...
<body>
<h4>서버에서 클라이언트로 전송되는 응답 형태 : HttpServletRequest</h4>
<pre>
	1. Response Line : status code
		...
		3) 300~ : response body
			...
			302/307 : Moved, 자원의 새로운 위치를 대상으로 새로운 요청을 전송함. redirect		location
			
		4) 400~ : client side error
			400 : <%=HttpServletResponse.SC_BAD_REQUEST %> : 필수 파라미터 누락, 요청 데이터 타입 문제, 요청 데이터 길이가 너무 길때... (요청 검증시 의도적으로 검증)
			401/403 : <%=HttpServletResponse.SC_UNAUTHORIZED%> / <%=HttpServletResponse.SC_FORBIDDEN %> 보안적 리소스를 관리할때 권한 없는 사용자가 접근했을 경우
			404 : <%=HttpServletResponse.SC_NOT_FOUND %> URL이 잘못되었을때
			405 : <%=HttpServletResponse.SC_METHOD_NOT_ALLOWED %> Server Side에 Call Back Method가 정의되지 않았을 때
	...
	2. Response Header (name/value)
		...
		2) Cache-Control : 캐시데이터 제어
		3) Refresh	: auto-request
		4) Location : 자원의 위치가 새로운 곳으로 이동한 경우, 새로운 위치 정보를 제공하는 해더(sendRedirect)
